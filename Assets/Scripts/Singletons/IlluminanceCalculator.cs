...
public class IlluminanceCalculator
{
	private static IlluminanceCalculator sharedInstance;
	...
	public static IlluminanceCalculator Instance()
	{
		if (sharedInstance == null)
			sharedInstance = new IlluminanceCalculator();
		return sharedInstance;
	}

	private IlluminanceCalculator()
	{
		...
		this.pointIlluminances = new List<float>();
		for (int i = 0; i < this.longCalPoints * 3; i++)
        {
			this.pointIlluminances.Add(0);
        }
	}
	...
	public void LoadIntensityFile(TextAsset file)
	{
		this.intensityTable = DataParser.Instance().ParseIntDataTable(file);
	}
}
