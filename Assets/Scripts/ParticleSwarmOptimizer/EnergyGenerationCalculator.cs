...
public class EnergyGenerationCalculator : MonoBehaviour
{
    [SerializeField] private TextAsset azimuthFile;
    [SerializeField] private TextAsset solarGenerationFile;
    [SerializeField] private TextAsset horizonAngleFile;
    ...
    void Awake()
    {
        azimuth = DataParser.Instance().ParseFloatDataTable(azimuthFile);
        horizonAngle = DataParser.Instance().ParseFloatDataTable(horizonAngleFile);
        solarGeneration = DataParser.Instance().ParseFloatDataTable(solarGenerationFile);
    }
	...
	private void AssignEnergyGenerationToSolar(SolutionParticle particle)
	{
		...
		for (int i = 0; i < particle.luminaireCount; i++)
		{
			lampList[i].GetComponentInChildren<AverageScore>().setScore(this.energyGeneration[i] * 0.607f);
			this.energyGeneration[i] = 0;
		}
	}
	/// <summary>
